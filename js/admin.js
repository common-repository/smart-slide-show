...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ sss_upload_dir+"/"+file.name + '&task=sss_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	sss_refresh_images_table(sss_current_album_id);
...
function sss_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=sss_get_albums_table', function(res)
	{
		sss_set_data_tables();
		if (res.status == 'ok') 
		{
...
function sss_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=sss_get_albums_images_table&album_id='+album_id, function(res)
	{
		sss_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'sss_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'sss_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'sss_reorder_image' )
		{
			sss_refresh_images_table(album_id);
		}
		if( task == 'sss_reorder_album' )
			sss_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function sss_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', sss_submit_bulk);
...
function sss_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'sss_bulk_delete_albums' || task == 'sss_bulk_enable_albums' || task == 'sss_bulk_disable_albums')
	{
	...
	else if( task == 'sss_bulk_delete_images' || task == 'sss_bulk_enable_images' || task == 'sss_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = sss_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: sss_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: sss_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: sss_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: sss_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				...
				upload_url: sss_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":sss_upload_dir, "secw":sss_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: sss_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.sss_delete_album').live('click', sss_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', sss_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', sss_disable);
